...
def test_parse():
    ...
    expected_keys = ['Previous Close', 'Open', 'Bid', 'Ask', "Day's Range", '52 Week Range', 'Volume', 'Avg. Volume', 'Market Cap', 'Beta (5Y Monthly)', 'PE Ratio (TTM)', 'EPS (TTM)', 'Earnings Date', 'Forward Dividend & Yield', 'Ex-Dividend Date', '1y Target Est', 'EPS Beat Ratio', 'ticker', 'url']
    parsed = []
...
def test_finance_formulas():
    test_convert_to_real_return()


def test_convert_to_real_return():
    assert convert_to_real_return(.08, 0.05) == 0.02857142857142857
